...
--	emu.Log(string.format("gulButtonHelpMask = %d", gulButtonHelpMask))
	...
	cpu.WriteMem32(0x800EE93C, 0x800EC2B8)
end
...
--	emu.Log(string.format("buttons = %08x", buttons))
	...
	if showingConfirmation == false and (buttons & 0x1000) ~= 0 then
		cpu.SetPC(0x8006D704) -- vPauseExitX
		showingConfirmation = true
	elseif needsReinit == true then
		cpu.SetPC(0x80014268) -- vTLUpdateSelOptionsStone
		needsReinit = false
...
function install_title_hooks()
	hooks[1] = cpu.AddHook(0x800143B8, 0x34420008, vTLSetupSelOptionsStone)
	hooks[2] = cpu.AddHook(0x800143D8, 0x3C02800F, vTLUpdateSelOptionsStone)
	hooks[3] = cpu.AddHook(0x800636CC, 0x00002821, xHelpProcessChoiceSelectionNo)
	hooks[4] = cpu.AddHook(0x8004E644, 0xAF803698, xGameFadeUpdate)
	showingConfirmation = false
	needsReinit = false
end
...
cpu.AddHook(0x800A8E2C, 0x27BDFFD0, MR_LoadOverlay)
