-- Lua 5.3
-- Title:   Medievil - SCES-01493 (EU-DE)
...
--	emu.Log(string.format("gulButtonHelpMask = %d", gulButtonHelpMask))
	...
	cpu.WriteMem32(0x800EF5AC, 0x800ED940)
end
...
--	emu.Log(string.format("buttons = %08x", buttons))
	...
	if showingConfirmation == false and (buttons & 0x1000) ~= 0 then
		cpu.SetPC(0x8006DCB8) -- vPauseExitX
...
function install_title_hooks()
	...
	hooks[3] = cpu.AddHook(0x80063BA4, 0x00002821, xHelpProcessChoiceSelectionNo)
	hooks[4] = cpu.AddHook(0x8004EA10, 0xAF800584, xGameFadeUpdate)
	showingConfirmation = false
	needsReinit = false
end
...
cpu.AddHook(0x800A9000, 0x27BDFFD0, MR_LoadOverlay)



function boot() -- _start
	-- patch save game sku id
	cpu.WriteMemStrZ(0x800C6538, "BESCES-00311")
end

cpu.AddHook(0x80099A64, 0x3C02800F, boot)
