-- Lua 5.3
-- Title:   Medievil - SCES-00311 (EU-EN)
...
--	emu.Log(string.format("gulButtonHelpMask = %d", gulButtonHelpMask))
	...
	cpu.WriteMem32(0x800EF40C, 0x800ED7A4)
end
...
--	emu.Log(string.format("buttons = %08x", buttons))
	...
	if showingConfirmation == false and (buttons & 0x1000) ~= 0 then
		cpu.SetPC(0x8006DD18) -- vPauseExitX
...
function install_title_hooks()
	...
	hooks[3] = cpu.AddHook(0x80063C04, 0x00002821, xHelpProcessChoiceSelectionNo)
	hooks[4] = cpu.AddHook(0x8004EA50, 0xAF8005A0, xGameFadeUpdate)
	showingConfirmation = false
	needsReinit = false
end
...
cpu.AddHook(0x800A9060, 0x27BDFFD0, MR_LoadOverlay)
